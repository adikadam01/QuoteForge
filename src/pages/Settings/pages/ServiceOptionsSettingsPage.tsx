...
// Used only if there are zero services in the DB and zero saved service_categories —
// i.e. a genuinely empty/fresh install with nothing to derive from.
const DEFAULT_CATEGORY_SUBCATEGORY: Record<string, string[]> = {
  Designing: ["Logo Design", "Poster Design", "Branding", "UI/UX Design"],
  Marketing: ["Social Media Marketing", "Performance Ads", "SEO", "Influencer Marketing"],
  Development: ["Website Development", "Web App", "Mobile App"],
...
export function ServiceOptionsSettingsPage() {
  const { serviceOptions, setServiceOptions, services } = useApp();
  const [local, setLocal] = React.useState(serviceOptions);
  const [newCategory, setNewCategory] = React.useState("");
  const [newSubcategory, setNewSubcategory] = React.useState<Record<string, string>>({});
  ...
  // Build a category -> subcategory map straight from the actual services in
  // the DB, so anything that exists as a real service always shows up here,
  // even if it was never explicitly added via this settings page.
  const derivedFromServices = React.useMemo(() => {
  ...
  // Merge: saved/custom categories (local.service_categories) + derived-from-services,
  // union of subcategories per category. This means categories/subcategories you added
  // manually are kept, and anything present in real service records is also shown.
  const categoryMap = React.useMemo(() => {
    const saved = local.service_categories && Object.keys(local.service_categories).length > 0
      ? local.service_categories
      : {};

    const hasAnySource =
      Object.keys(saved).length > 0 || Object.keys(derivedFromServices).length > 0;

    const base = hasAnySource ? {} : DEFAULT_CATEGORY_SUBCATEGORY;

    const merged: Record<string, string[]> = {};

    const addAll = (source: Record<string, string[]>) => {
      for (const [cat, subs] of Object.entries(source)) {
    ...
    addAll(base);
    addAll(derivedFromServices);
    addAll(saved);

    return merged;
  }, [local.service_categories, derivedFromServices]);

  const save = async () => {
    // Persist the full merged map (including anything derived from services)
    // so it's captured going forward, plus any manual edits.
    await setServiceOptions({
      ...local,
      service_categories: categoryMap,
    });
